...
export default function Appointments() {
  ...
  const listUrl = useMemo(() => {
    ...
    if (selectedPatient?.id) params.set("patient", selectedPatient.id);
    return `${endpoint.lab.appointments.list}?${params.toString()}`;
  }, [tab, page, dateFrom, dateTo, selectedPatient]);
  ...
  const hasFilters =
    tab !== "all" || !!dateFrom || !!dateTo || !!selectedPatient || branchFilter !== "all";
  ...
  return (
    <div className="space-y-5 animate-fade-in">
            ...
            <div className="flex items-center gap-0.5 bg-muted rounded-lg p-1">
              {STATUS_TABS.map((t) => (
                <button
                  ...
                  className={`px-3 py-1.5 text-xs font-semibold rounded-md transition-all whitespace-nowrap ${tab === t
      ...
      {
        showBookDialog &&
        <BookDialog open={showBookDialog} onClose={() => setShowBookDialog(false)} />
      }
      <DetailSheet
